...
import ru.ncideas.magazinof.gwt.client.util.FastMap;
...
public class BaseObservable implements Observable {
    ...
    public boolean fireEvent(EventType eventType, BaseEvent be) {
        if (firesEvents && listeners != null) {
            ...
            List<Listener<BaseEvent>> list = listeners
                    .get(getKey(eventType));
            if (list != null) {
                List<Listener<BaseEvent>> copy = new ArrayList<Listener<BaseEvent>>(
                        list);
    ...
    public void removeListener(final EventType eventType,
                               final Listener<? extends BaseEvent> listener) {
        if (listeners == null) {
            return;
        }
        String key = getKey(eventType);
